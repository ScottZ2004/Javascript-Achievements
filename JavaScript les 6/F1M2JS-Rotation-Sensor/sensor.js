...
function activateVoice(alpha, beta, gamma){
    ...
    if(alpha > 120){
        let myTxt = "Stop it. get some help.";  
        sayItLoud(myTxt); // text 2 speech

        // laat het ook in de HTML zien
        document.getElementById('voice').innerHTML = myTxt;
    }else if(gamma > 40){
        let myTxt2 = "Gamaaalaalaaalaaaaaaaaalaaaaaalaaaaaaalaaaalaaaaalaalaaalaaaaaaaaaaaalaaaaaaaaalaaaaaaalaaaaaaaalaaaaokepapaslamenietvandaagalstjeblieft"
...
function showMyImage(alpha, beta, gamma){
    ...
    if(beta > 40){
        document.getElementById('myImage').src = "Minecraft_cover.png"; // show image
    }else if(alpha > 120) {
